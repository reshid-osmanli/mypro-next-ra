...
export async function getProductBySlug(slug: string) {
  try {
    ...
    console.warn("[catalog] Unable to load product by slug", error);
    return null;
  }
}
...
export async function getPageByGradeSubject(grade: string, subject: string) {
  try {
    return await prisma.contentPage.findFirst({ where: { grade, subject, published: true } });
  } catch (error) {
    console.warn("[catalog] Unable to load content page", error);
    return null;
  }
}
